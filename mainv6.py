...
# alien move buffer variable will be used to delay alien movement (this is to make the game easier as first version was absurdly hard)
# initialized at 0, will count up and be reset periodically
alien_move_buffer = 0
...
'''
# global variable for bullet spawn offset (to center the spawn location on the center of the player sprite) dividing by 3.5
# was experimentally determined 
bullet_offset = player_size[0]/3.5
...
class ship:
    ...
    max_x = 1200
    min_x = 0
    image = pygame.image.load('player.png') #image for the ship, we decided to use a face (only non edited face as the aliens are heavily edited faces)
    # rescale image
...
class alien:
    ...
    y_change = 50
    health = 1 # number of shots it will take to kill
    ...
    def move(aliens):
        ...
        # iterator variable and globals move tracker and alien move buffer
        i = 0
        ...
        if alien_move_buffer == 50:
            ...
            # if no move went through, increment the buffer
            alien_move_buffer += 1
        
'''
Bullet class done by RP
'''
# BULLET OBJECT
class bullet:
...
'''
spawn_aliens function done by EA
'''
...
def spawn_aliens():
    ...
    '''
    General logic for spawning in the aliens: 
        - Need to initialize each alien's starting position (logic for this below)
        - Initialize speed (same equation as in the class, but needs to occur here to change the speed every level)
        - Initialze sprite (also same as class and occurs for each alien to generate a random sprite for each new alien)
        - Direction does NOT have to be initialized, as it is always the same no matter the level 
        - All this is done by creating a new alien object, initializing it and appending it to the aliens array
    '''
    ...
    if starting_aliens != 0:
        ...
        # assign speed
        new_alien.speed = -math.pow(level, 2)/2
...
'''
display_aliens function done by SC
'''
    ...
    while i < starting_aliens:
        ...
        if aliens[i].x_pos - bullet_size[0] < bullet.x_pos and aliens[i].x_pos + alien_size[0] > bullet.x_pos:
            ...
            if aliens[i].y_pos - alien_size[1] <= bullet.y_pos and aliens[i].y_pos - alien_size[1]/2 >= bullet.y_pos:
                ...
                score += level
                num_aliens -= 1
                # set firing to false as bullet is no longer in a firing sequence
                firing = False

                # bullet and alien hit need to disappear. Hide them at (2000, 0)
                # ensures object is hidden and does not interfere with game_over code (which checks if alien passes y_pos of player)
                bullet.x_pos = 2000
                bullet.y_pos = 0
                aliens[i].x_pos = 2000
                aliens[i].y_pos = 0
                ...
                # wait for key press and release to continue
                    if event.type == pygame.KEYUP:
                        if event.key == pygame.K_q:
                            ...
                            num_aliens = 0
                            starting_aliens = 0
                            # this is modified on some level ups, so reset it as well
                            level_msg_pos = (250, 200)
                            # paint the board black to get rid of game over message
                            board.fill(black)
                        wait = False
        
        # break the loop if a loss is detected
        if lost:
            break
        i += 1
...
'''
creation of objects done by KF
'''
# create objects
player = ship()
projectile = bullet()
...
'''
code to loop during game and quit when player exits done by SC
'''
...
while running:
    ...
    '''
    game_loop code done by KF and SC (from checking if firing == True to checking for game_over)
    '''
    ...
    # display the player and projectile
    display(player)
    display(projectile)
    # show the score
    show_score()
    ...
    '''
    checking if all aliens destroyed and implementing level up done by EA
    '''
    ...
    if num_aliens == 0:
        ...
        # delete and re initialize aliens array, this is done so the spawn_aliens function has a blank new array to fill
        del aliens
        aliens = []
        spawn_aliens()
